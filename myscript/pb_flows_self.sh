...
do
  ...
  if [ $PORT_NUM -eq 1 ]; then
   ...
   if [ $FACTOR -eq 0 ]; then
    ...
    ((PEER_AC_ID=$AC_ID - 1))
    ((SRC_VOQ=$VOQ_BASE + ($SRC_SLOT_RELATIVE-1)*VOQ_NUM_PER_SLOT + ($port-2)*VOQ_NUM_PER_PORT))
   else
    ((DST_TM_PORT=$SRC_TM_BASE + $port))
    ((PEER_AC_ID=$AC_ID + 1))
    ((SRC_VOQ=$VOQ_BASE + ($SRC_SLOT_RELATIVE-1)*VOQ_NUM_PER_SLOT + ($port)*VOQ_NUM_PER_PORT))
   fi
  fi
